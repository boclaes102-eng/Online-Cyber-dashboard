...
// Used client-side where Web Crypto doesn't support MD5

function safeAdd(x: number, y: number): number {
  const lsw = (x & 0xffff) + (y & 0xffff)
  return ((x >> 16) + (y >> 16) + (lsw >> 16)) << 16 | (lsw & 0xffff)
}
function bitRol(n: number, c: number): number { return n << c | n >>> (32 - c) }
...
function rhex(n: number): string {
  ...
  return h[(n >> 4) & 0xf] + h[n & 0xf] +
         h[(n >> 12) & 0xf] + h[(n >> 8) & 0xf] +
         h[(n >> 20) & 0xf] + h[(n >> 16) & 0xf] +
         h[(n >> 28) & 0xf] + h[(n >> 24) & 0xf]
}
...
export function md5Buffer(buf: ArrayBuffer): string {
